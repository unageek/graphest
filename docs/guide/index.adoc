...
For stem:[x ≥ 0], both `x^y` and `x^^y` gives the same value, stem:[x] raised to the power of stem:[y]. For stem:[x < 0], `x^y` is only defined for integer exponents, while `x^^y` is also defined for rational number exponents with odd denominators. The exact definition of these operators are as follows.

* For an integer stem:[n], both `x^n` and `x^^n` gives:
+
...
* For a non-integer stem:[y]:

** `x^y` gives the principal value of stem:[e^{y \ln x}].
...
** Otherwise, `x^^y` gives the principal value of stem:[e^{y \ln x}], thus the same as `x^y`.
...
|https://functions.wolfram.com/ElementaryFunctions/Sinc/[MFS]
