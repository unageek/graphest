...
impl FftImage {
    pub fn new(width: u32, height: u32) -> Self {
        ...
        let ptr = unsafe { fftw_alloc_real(len as u64) };
        // The arrays are destroyed during planning. See
        //   https://www.fftw.org/fftw3_doc/Planner-Flags.html
        let plan_r2c = unsafe {
            fftw_plan_dft_r2c_2d(height as i32, width as i32, ptr, ptr as *mut [f64; 2], 0)
        };
        let plan_c2r = unsafe {
            fftw_plan_dft_c2r_2d(height as i32, width as i32, ptr as *mut [f64; 2], ptr, 0)
        };
    ...
    pub fn complexes(&self) -> &[[f64; 2]] {
        unsafe { from_raw_parts(self.ptr as *const [f64; 2], self.len / 2) }
    }

    pub fn complexes_mut(&mut self) -> &mut [[f64; 2]] {
        unsafe { from_raw_parts_mut(self.ptr as *mut [f64; 2], self.len / 2) }
    }

    pub fn fft(&mut self) {
        unsafe { fftw_execute(self.plan_r2c) };
    }

    pub fn ifft(&mut self) {
        unsafe { fftw_execute(self.plan_c2r) };
    }
}
...
impl Drop for FftImage {
    fn drop(&mut self) {
        unsafe {
            fftw_free(self.ptr as *mut c_void);
