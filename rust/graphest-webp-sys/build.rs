...
const WEBP_GIT_TAG: &str = "v1.2.2";
const WEBP_GIT_URL: &str = "https://github.com/webmproject/libwebp.git";
...
fn build_webp(env: &Environment) {
    if env.lib_dir.join("libwebp.a").exists() {
        return;
    }

    let build_dir = env.build_dir.join("webp-build");
    if !build_dir.exists() {
    ...
    fs::create_dir_all(&build_dir)
        .unwrap_or_else(|_| panic!("failed to create the directory: {:?}", build_dir));
...
fn run_bindgen(env: &Environment) {
    let binding_file = env.out_dir.join("webp.rs");
